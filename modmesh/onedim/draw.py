...
"""
Drawing utilities one-dimensional space-time conservation element and solution
element (CESE) method
"""
...
class PstCanvas:
    ...
    def __init__(self, unit=None, padding=0, extent=None, show_grid=False):
        ...
        if isinstance(extent, collections.abc.Sequence):
            if 2 == len(extent):
    ...
    def selm(self, se, time, **kw):
        ...
        # Upside.
        cmds.append(
            self.line((x + sep2, y + sep3), (x + sep2, yp - sep4), **kw))
        cmds.append(
            self.line((x - sep2, y + sep3), (x - sep2, yp - sep4), **kw))
        cmds.append(self.arc((x, yp - sep), sep2, 0, 180, **kw))
        # Downside.
        cmds.append(
            self.line((x + sep2, y - sep3), (x + sep2, yn + sep4), **kw))
        cmds.append(
            self.line((x - sep2, y - sep3), (x - sep2, yn + sep4), **kw))
        cmds.append(self.arc((x, yn + sep), sep2, 180, 0, **kw))
        # Rightside.
        cmds.append(
            self.line((x + sep3, y + sep2), (xp - sep4, y + sep2), **kw))
        cmds.append(
            self.line((x + sep3, y - sep2), (xp - sep4, y - sep2), **kw))
        cmds.append(self.arc((xp - sep, y), sep2, 270, 90, **kw))
        # Leftside.
        cmds.append(
            self.line((x - sep3, y + sep2), (xn + sep4, y + sep2), **kw))
        cmds.append(
            self.line((x - sep3, y - sep2), (xn + sep4, y - sep2), **kw))
