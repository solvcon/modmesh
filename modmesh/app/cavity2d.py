...
# benchmark
x_b = [0, 0.0625, 0.0703, 0.0781, 0.0938, 0.1563, 0.2266, 0.2344, 0.5000, 0.8047, 0.8594, 0.9063, 0.9453, 0.9531, 0.9609, 0.9688, 1.0]  # noqa: E501
y_b = [0, 0.0547, 0.0625, 0.0703, 0.1016, 0.1719, 0.2813, 0.4531, 0.5000, 0.6172, 0.7344, 0.8516, 0.9531, 0.9609, 0.9688, 0.9766, 1.0]  # noqa: E501
Re_100_v = [0, 0.09233, 0.10091, 0.10890, 0.12317, 0.16077, 0.17507, 0.17527, 0.05454, -0.24533, -0.22445, -0.16914, -0.10313, -0.08864, -0.07391, -0.05906, 0]  # noqa: E501
Re_100_u = [0, -0.03717, -0.04192, -0.04775, -0.06434, -0.10150, -0.15662, -0.21090, -0.20581, -0.13641, 0.00332, 0.23151, 0.68717, 0.73722, 0.78871, 0.84123, 1]  # noqa: E501
...
def meet_poisson():
    ...
    for i in range(1, N):
        for j in range(1, N):
            ...
            poisson_RHP = (P[i + 1, j] + P[i - 1, j] + P[i, j + 1] + P[i, j - 1] - 4 * P[i, j]) / (dx * dx)  # noqa: E501
...
def collocate():
    for i in range(N):
        for j in range(N):
            ...
            PC[i, j] = (P[i, j] + P[i + 1, j] + P[i, j + 1] + P[i + 1, j + 1]) * 0.25  # noqa: E501
...
def moniter(timestep):
    ...
    ax[1].set(xlim=(0, 1))
    ax[2].imshow(np.transpose(np.sqrt(np.multiply(UC, UC) + np.multiply(VC, VC))))  # noqa: E501
    ax[2].invert_yaxis()
    plt.show()
    ...
    while not meet_poisson():
        ...
        for i in range(1, N):
            for j in range(1, N):
                ...
                vy = (V_2[i, j] - V_2[i, j - 1]) / dy
                poisson_LHP = (ux + vy) / dt
                P[i, j] = 0.25 * (P[i + 1, j] + P[i - 1, j] + P[i, j + 1] + P[i, j - 1] - poisson_LHP * dx * dx)  # noqa: E501
    meet_poisson()
