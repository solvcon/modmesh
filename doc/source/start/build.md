...
than system-wide, use the standalone mmdv build scripts in `contrib/dependency/`
described below.

For a complete, self-contained environment, the per-platform scripts
`ubuntu2404/build-mmdv-ubuntu24.sh` and `macos26/build-mmdv-macos26.sh` build
...
`${HOME}/var/mmdv/<platform>-py<pyver>-qt<qtver>`).

The build is organized into four sections: `BASE`, `PYTHON`, `NUMPY`, and `QT`
with the corresponding environment variables `MMDVBUILD_BASE`,
`MMDVBUILD_PYTHON`, `MMDVBUILD_NUMPY`, and `MMDVBUILD_QT`. If none is set, the
...
./build-mmdv-ubuntu24.sh --print-apt   # review, then run the printed commands
./build-mmdv-ubuntu24.sh               # build the whole stack into the prefix
```
...
```sh
source ${HOME}/var/mmdv/<platform>-py<pyver>-qt<qtver>/activate
```
